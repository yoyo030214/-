...
// 商家关联
Merchant.hasMany(MerchantProduct, { foreignKey: 'merchant_id', as: 'products' });
MerchantProduct.belongsTo(Merchant, { foreignKey: 'merchant_id', as: 'merchant' });

Merchant.hasMany(MerchantOrder, { foreignKey: 'merchant_id', as: 'orders' });
MerchantOrder.belongsTo(Merchant, { foreignKey: 'merchant_id', as: 'merchant' });

MerchantOrder.hasMany(MerchantOrderItem, { foreignKey: 'order_id', as: 'items' });
MerchantOrderItem.belongsTo(MerchantOrder, { foreignKey: 'order_id', as: 'order' });
...
const syncDatabase = async () => {
  try {
    ...
    // 同步所有模型到数据库
    await sequelize.sync({ force: false }); // 生产环境不要使用 force: true
